...
if(isset($_POST['update'])) {
   //$username='root';
    ...
    //echo var_dump($_POST["application_ref"]);
    //echo var_dump($_POST["company_reg_no"]);
    //echo var_dump($_POST["loan_number"]);
    ...
    //else
    if ($application_ref !== "" ) {
	  $Update_CIF_Query ="UPDATE loan_data 
	   SET    cif='$cif' 
    ...
    if ($loan_number !== "") {
	   $Update_CIF_Query ="UPDATE loan_data 
	   SET    cif='$cif' 
	   WHERE  company_reg_no = '$company_reg_no' and loan_number='$loan_number'"; 
    }
    ...
    mysqli_select_db($con,"corporatescoring");
    $Update_Success =mysqli_query($con,$Update_CIF_Query);
    if  ($Update_Success){
        //echo $application_ref;
  
	    $count=mysqli_affected_rows($con);
		
		if ($count>0){
		   echo '<script>';
	       echo 'alert ("CIF ..successfully updated")';
	       echo '</script>';
        }
        else{
           echo '<script>';
		   echo 'alert ("Company record not found")';
	       echo '</script>';	    
        }		
    }
 }
...
-->
<TITLE>CREDIT SCORING - Get New Company Registration Number To Edit</TITLE>
  ...
  {	  
	...
	**/
	...
	//otherwise validate the inputs
	  ...
	  if (document.GetRegNo.cif.value == "") {
		  alert ("The CIF cannot be empty");
	      return false;
	  }
...
<table class="s_text">
<tr>
    <td><strong><em><font color="#FF9900" size="1">&nbsp;&nbsp;&nbsp;&nbsp;Enter company's loand identification details below</font></em></td>
</tr>
<tr>
...
<tr>
    <td>CIF:</td>
	<td><input name = "cif" type = "number"></td>
</tr>
<tr><td>&nbsp;  </td></tr>
<tr> 
<td align="right">
