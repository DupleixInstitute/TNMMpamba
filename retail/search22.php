			...
			$.ajax({
				...
				success: function(result){
					if (result.errorStatus == 'Error') {
						...
						$("#first_level_approval").attr('action', '');			
					} else {
						if (result.data.loan_category == 'mortgage') {
								//alert(document.getElementById('first_level_approval').action);
							$("#first_level_approval").attr('action', 'viewform_mortgage.php');		 
								//swal (result.data.borrower_name,result.data.loan_category,'success');
								//form.action = 'viewform_mortgage.php';
						} else {
							$("#first_level_approval").attr('action', 'viewform_unsecured_loan.php');
						}
						document.getElementById('first_level_approval').submit();								
					}	   
				}
			});
		}	
					...
					<tr>
						<table class="s_text" bgcolor = "yellow">
							<tr bgcolor="#ECF8FF" class="s_text"> 
								<td>
									<p></br> 
									<form ACTION="" name="first_level_approval" id="first_level_approval" method="post">
										<td>
											<table>
												<tr>
													<td><p align="justify" class="s_text"><br>
														<table class="s_text">
															...
															<tr><td>&nbsp;&nbsp;&nbsp;Omang/Passport Number :&nbsp;&nbsp;
																	<input name="omang" id = "omang" type="text" required >
																</td>
																<td>&nbsp;&nbsp;&nbsp;Loan Number</td>
																<td>
																	<SELECT name="loan_number" id = "loan_number" required>
																		<option value=""> </option>
