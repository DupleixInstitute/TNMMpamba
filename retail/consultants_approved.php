...
$sql1="select count(*) as count,cust_information.username as consultant,approvals.status as status from cust_information,approvals where cust_information.omang_passport_num=approvals.omang and  approvals.status='approved' and cast(approvals.date as date)=$year$month$day  group by cust_information.username;";
}

if (isset($day2)) {

$sql1="select count(*) as count,cust_information.username as consultant,approvals.status as status from cust_information,approvals where cust_information.omang_passport_num=approvals.omang and  approvals.status='approved' and cast(approvals.date as date)=$year2$month2$day2  group by cust_information.username";
}


if(isset($day) && isset($day2))
{
$sql1="select count(*) as count,cust_information.username as consultant,approvals.status as status from cust_information,approvals where cust_information.omang_passport_num=approvals.omang and  approvals.status='approved' and cast(approvals.date as date) between $year$month$day and $year2$month2$day2  group by cust_information.username;";

}

if(empty($day) && empty($day2)){ 
$sql1="select count(*) as count,cust_information.username as consultant,approvals.status as status from cust_information,approvals where cust_information.omang_passport_num=approvals.omang and  approvals.status='approved' group by cust_information.username";
}
...
<td width="1286" align="CENTER" bgcolor="#000040" class="s_text"><font  color="#FFFFFF" size="7" face="Arial Narrow">CREDIT SCORING</font></td><tr>
   ...
   <table class="s_text" >
   <tr><td><font color="#000066" size="4" class="btn"><strong>APPROVALS BY CONSULTANTS</strong></font></td></tr>
