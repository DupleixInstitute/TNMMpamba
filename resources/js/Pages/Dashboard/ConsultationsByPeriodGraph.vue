<template>
    <div class=" max-h-full w-full bg-white rounded mb-6 xl:mb-0 shadow-lg">
        ...
        <div class="mb-2  p-2">
            <bar :chart-data="consultationsByPeriodData" ref="consultationsByPeriodData"
...
export default {
    ...
    mounted() {
        this.getConsultationsByPeriod();

    },
    methods: {
        getConsultationsByPeriod() {
            let query = pickBy(this.form)
            axios.get(this.route('dashboard.get_consultations_by_period_graph', Object.keys(query).length ? query : {})).then(response => {
                let labels = [];
                let data = [];
                ...
                this.consultationsByPeriodData = {
                    labels: labels,
                    datasets: [{
                        label: 'Consultations',
                        data: data,
