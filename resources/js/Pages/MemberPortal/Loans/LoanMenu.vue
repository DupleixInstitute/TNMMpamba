    ...
    <div>
        ...
        <div class="w-full mt-5">
            <inertia-link
                ...
                :class="{'bg-gray-100': route().current('portal.loans.show')}"
                :href="route('portal.loans.show',loan.id)">
                <font-awesome-icon icon="user" class="w-4 h-4 mr-2"></font-awesome-icon>Loan Details
            </inertia-link>
            <inertia-link
                          class="w-full border-t border-gray-100 font-medium text-gray-600 py-2 px-4 w-full block hover:bg-gray-100 transition duration-150"
                          :class="{'bg-gray-100': route().current('portal.loans.guarantors.index')}"
                          :href="route('portal.loans.guarantors.index',loan.id)">
                <font-awesome-icon icon="users" class="w-4 h-4 mr-2"></font-awesome-icon>Guarantors
            </inertia-link>
            <inertia-link
                          class="w-full border-t border-gray-100 font-medium text-gray-600 py-2 px-4 w-full block hover:bg-gray-100 transition duration-150"
                          :class="{'bg-gray-100': route().current('portal.loans.notes.index')}"
                          :href="route('portal.loans.notes.index',loan.id)">
                <font-awesome-icon icon="bookmark" class="w-4 h-4 mr-2"></font-awesome-icon>
                Notes
            </inertia-link>
            <inertia-link
                          class="w-full border-t border-gray-100 font-medium text-gray-600 py-2 px-4 w-full block hover:bg-gray-100 transition duration-150"
                          :class="{'bg-gray-100': route().current('portal.loans.files.*')}"
                          :href="route('portal.loans.files.index',loan.id)">
