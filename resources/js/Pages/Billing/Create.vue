                        ...
                        <div class="mt-4 mb-4">
                            <table class="w-full whitespace-no-wrap">
                                ...
                                <tr v-for="(item,index) in form.items">
                                    ...
                                    <td class="border-t">
                                        <jet-input type="text" class="mt-1 block w-full"
                                                   v-model="form.items[index].cash_amount"
                                                   required autocomplete="off" @input="updateItems"/>
                                    </td>
                                    <td class="border-t">
                                        <jet-input type="text" class="mt-1 block w-full"
                                                   v-model="form.items[index].co_payer_amount"
                                                   required autocomplete="off" @input="updateItems"/>
                                    </td>
                                    <td class="border-t">
                                        <jet-input type="text" class="mt-1 block w-full"
                    ...
                    this.taxRates.forEach(taxRate => {
                        if (taxRate.id === item.tax_rate_id) {
                            if (taxRate.type === 'fixed') {
                                ...
                                item.tax_total = parseFloat((item.unit_cost * item.qty * parseFloat(taxRate.amount)) / 100);
                            }
                        }
                    })
                }
                item.total = parseFloat(item.tax_total) + (parseFloat(item.unit_cost) * item.qty);
                item.cash_amount = parseFloat(item.total) - parseFloat(item.co_payer_amount);
